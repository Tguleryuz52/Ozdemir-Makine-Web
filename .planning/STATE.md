...
- Doğrulama: lint + `npm run build` temiz; footer DOM ölçüldü (font Geist 600, mavi çubuk rgb(1,113,185), 5 svg ikon), katalog başlığı 52px teyit; stok kartları yeni sıra+metin + detay "PDF İndir" butonu DOM'da doğrulandı. In-app pane reveal donması yüzünden görsel screenshot güvenilmez → DOM ölçümüyle doğrulandı; Talha kendi Chrome'unda canlı görüyor. `category-cards.tsx` artık sadece `CategoryTile` kaynağı. Commit/push YAPILMADI (Talha onayı bekler).
